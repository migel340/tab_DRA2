...
export const ClientAddressFormSchema = z.object({
  city: z.string().trim().min(1, "Miasto jest wymagane").max(20).regex(/^[a-zA-ZąćęłńóśźżĄĆĘŁŃÓŚŹŻ\s]+$/, "Miasto może zawierać tylko litery i spacje"),
  state: z.string().trim().min(1, "Województwo jest wymagane").max(20).regex(/^[a-zA-ZąćęłńóśźżĄĆĘŁŃÓŚŹŻ\s\-]+$/, "Województwo może zawierać tylko litery, spacje i myślniki"),
  postalCode: z.string().trim().min(1, "Kod pocztowy jest wymagany").max(6).regex(/^\d{2}-\d{3}$/, "Nieprawidłowy kod pocztowy"),
  country: z.string().trim().min(1, "Kraj jest wymagany").max(20).regex(/^[a-zA-ZąćęłńóśźżĄĆĘŁŃÓŚŹŻ\s]+$/, "Kraj może zawierać tylko litery i spacje"),
});
...
const baseClientFormFields = z.object({
  ...
  secondName: z.string().trim().regex(/^[a-zA-ZąćęłńóśźżĄĆĘŁŃÓŚŹŻ]+$/, "Drugie imię może zawierać tylko litery").or(z.literal("")).optional(),
